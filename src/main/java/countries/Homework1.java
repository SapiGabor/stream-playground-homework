...
public class Homework1 {
    ...
    public boolean streamPipeline1() {
        return countries.stream().map(c -> c.getName()).anyMatch(n -> n.toLowerCase().contains("island"));
    }
    ...
    public Optional<String> streamPipeline2() {
        return countries.stream().map(c -> c.getName()).filter(c -> c.toLowerCase().contains("island")).findFirst();
    }
    ...
    public void streamPipeline3() {
        countries.stream().map(c -> c.getName()).filter(c -> c.toLowerCase().charAt(0) == c.toLowerCase().charAt(c.length()-1)).forEach(System.out::println);
    }
    ...
    public IntSummaryStatistics streamPipeline6() {
        return countries.stream().mapToInt(c -> c.getTranslations().size()).summaryStatistics();
    }
